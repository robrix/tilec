...
let'''
  :: (Applicative m, S.Let v expr)
  => m expr ::: m expr
  -> (forall i . Permutable i => i v -> (m :.: i) expr)
  -> m expr
let''' (tm ::: ty) f = S.let' <$> ((:::) <$> tm <*> ty) <*> getC (f id)
...
lam' :: (Applicative m, S.Lam v expr) => m Plicit -> (forall i . Permutable i => i v -> (m :.: i) expr) -> m expr
lam' p f = S.lam <$> p <*> getC (f id)

...
let_ :: (MonadGen m, MonadReader Int m, Let v a, MonadWriter (Set LabelName) m) => m a -> m a
let_ t = Gen.subtermM2 t (local succ t) (\ v b -> let' v (const b) <$ tag "let")

lam_ :: (MonadGen m, MonadReader Int m, Lam v a, MonadWriter (Set LabelName) m) => m a -> m a
...
pi_ :: (MonadGen m, MonadReader Int m, Type v a, MonadWriter (Set LabelName) m) => m a -> m a
pi_ t = Gen.subtermM2 t (local succ t) (\ t b -> (>-> const b) . (, t) <$> plicit <* tag ">->")

ex_ :: (MonadGen m, MonadReader Int m, Prob v a, MonadWriter (Set LabelName) m) => m a -> m a
ex_ t = Gen.subtermM2 t (local succ t) (\ t b -> (t `ex` const b) <$ tag "ex")

eq_ :: (MonadGen m, Prob v a, MonadWriter (Set LabelName) m) => m a -> m a
